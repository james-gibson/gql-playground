...
const Query = objectType({
    name: 'Query',
    definition(t) {
        t.nonNull.list.nonNull.field('allUsers1', {
            type: 'User',
...
const Mutation = objectType({
    ...
    definition(t) {
        ...
        t.field('incrementDishViewCount1', {
            type: 'Dish',
            args: {
                id: nonNull(intArg()),
            },
...
const dishServiceSchema = makeSchema({
    types: [
        Query,
        Mutation,
        Dish,
...
export const schema = transformSchemaFederation(dishServiceSchema, {
    Query: {
        extend: true,
    },
    User: {
        ...
        resolveReference(reference, context, info) {
            return delegateToSchema({
                schema: info.schema,
                operation: 'query',
                fieldName: 'draftsByUser',
                args: {
                    id: (reference as UniqueUser).id,
                },
                context,
                info,
            });
